...
const NewsTable = ({ data, setData, onPageChange }) => {
  ...
  if (typeof window !== "undefined") {
     ...
     userId=JSON.parse(localStorage.getItem("cnnUser"))?._id
  }
  //const userRole=JSON.parse(localStorage.getItem("userRole"))
  //const userId=JSON.parse(localStorage.getItem("cnnUser"))?._id
  let filteredData 
 if(!userRole==="Admin"){
   filteredData = data?.filter(
    (item) =>
     item.repoter===userId&& item?.title && item?.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
 }else{
  filteredData = data?.filter(
    (item) =>
        ...
        <table className="min-w-full bg-white border border-gray-300">
          <thead>
            <tr>
              ...
              <th className="py-2 px-4 border-b text-start">Published</th>
          ...
          <tbody>
            {paginatedData.map((item) => (
              <tr key={item?._id}>
                ...
                <td className="py-2 px-4 border-b">{item?.pb===false?<small style={{color:"red",}}><b>Pending</b></small>:<small style={{color:"green",}}><b>Published</b></small>}</td>
                ...
                <td className="py-2 px-4 border-b flex self-end justify-end">
                  <button>
                    <Link
                      href={`/admin/news-management/update/${item?._id}`}
                  ...
                  </button>
                  {
                    ...
                    <div>
                      <p className="mx-1 my-1 btn btn-sm"><Link href={`/news/${item?._id}`}>View</Link></p>
                       {
                        !item?.pb===true?<button className="btn btn-sm" onClick={()=>handleApprove(item?._id,1)}>Approve</button>:
                        <button className="btn btn-sm" onClick={()=>handleApprove(item?._id,2)}>Cancel</button>
                       }
                    </div>
                  } 
                </td>
